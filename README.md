...
[`Website`](https://sot.epfl.ch) | [`arXiv (comming soon)`](https://sot.epfl.ch) | [`🤗 Demo (coming soon)`](https://sot.epfl.ch) | [`BibTeX`](#citation)
...
![SoT pull figure](./assets/pull_figure.png)
...
Building on this study, we release `SoT`, a modular framework for test-time search in autoregressive image generation, designed to support the broader research community.
...
![SoT pull figure](./assets/framework.png)
...
```python
from sot.ar_priors.base import ARPriorFactory
from sot.search_algorithms.base import SearchAlgorithmFactory
...
| [`04_other_ar_models.ipynb`](notebooks/04_other_ar_models.ipynb) | Run best-of-N, beam, and lookahead search on Janus-Pro 7B and Infinity 2B, demonstrating SoT generalizes across AR model families. |
...
torchrun --nproc_per_node=4 sot/run_search.py --config-name eval/flextok/geneval_flextok_ar_3b_beam_ir

# Custom prompts (override dataset with inline prompts)
python sot/run_search.py \
...
---
## Code Structure
This repository consists of two main components: (1) FlexTok Autoregressive (AR) Models and (2) Search-over-Tokens (SoT) Framework. The overall structure is shown below:
...
---
## SoT Framework
The SoT framework decouples test-time search for image generation into three independently swappable components. Each component is instantiated by name through YAML configuration files, making the framework modular and easily extensible.
...
`SoT` wraps multiple AR image generation models behind a unified `BaseARPrior` interface. Each model is registered by name and instantiated from a YAML config.
...
`SoT` provides three search strategies, all implementing `BaseSearchAlgorithm`.
...
```bash
bash sot/scripts/run.sh              # FlexTok beam search on GenEval (single GPU)
bash sot/scripts/run_ddp.sh 4        # Same, 4 GPUs
bash sot/scripts/run_bon.sh          # Best-of-50
bash sot/scripts/run_janus_pro.sh    # Janus Pro
bash sot/scripts/run_infinity.sh     # Infinity
```
