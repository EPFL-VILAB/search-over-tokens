...
# Default config directory: sot/configs/components/ar_priors/
_DEFAULT_CONFIG_DIR = Path(__file__).parent.parent.parent / "configs" / "components" / "ar_priors"

logger = logging.getLogger("sot.flextok")
...
class FlexTokARPrior(BaseARPrior):
    ...
    Integrates the flextok-ar package into the SoT framework.
    ...
    def __init__(self, config: Dict, device: str = "cuda"):
        ...
        #              a) SoT ar_prior YAML  →  ar_prior.model_id  (current format)
        #              b) legacy flextok cfg →  model.checkpoint
        #           When a SoT YAML is provided, its ar_prior.* values are merged
        ...
        if model_id is None:
            ...
            if checkpoint_path:
                ...
                if OmegaConf.select(cfg_temp, "ar_prior") is not None:
                    # SoT ar_prior YAML format: merge ar_prior.* then overlay explicit config
        ...
        # Search-specific overrides (replacement, return_probs, etc.) come from SoT config.
